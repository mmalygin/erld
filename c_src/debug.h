...
#define LOG(FMT) erld_log(1, 1, __FUNCTION__, FMT)
#define LOG_V(FMT, ...) erld_log(1, 1, __FUNCTION__, FMT, __VA_ARGS__)

void fatal_error(const char *file, int line, int e, const char *fmt, ...) __attribute__((format(printf, 4, 5)));
void erld_log(int prefix, int suffix, const char *fun, const char *fmt, ...) __attribute__((format(printf, 4, 5)));
